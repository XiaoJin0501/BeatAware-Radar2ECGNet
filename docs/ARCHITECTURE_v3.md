...
| 模式 | 解码器 | Table | 参数总量 |
|------|--------|-------|----------|
| `use_diffusion=False` | ConvTranspose×2 + Sigmoid | **Table 1** 主线 | **1,698,270** |
...
## § 6. Backbone (Multi-scale + Mamba + Conformer)
...
### GroupMambaBlock × 2
代码：`src/models/BeatAwareNet/radar2ecgnet.py:270-271`，模块 `src/models/backbone/group_mamba.py`

- 把 256 通道分成 4 组（每组 64 ch），各自跑 1D selective SSM
- $d_{\text{state}}=16$，长序列建模能力 ~ Transformer 但 O(L) 复杂度
- 串行 2 层
...
| Mamba | Transformer | 1600 长序列 + 受限 GPU；O(L) vs O(L²) |
| Conformer Fusion | 纯 Mamba | Mamba 是单向因果扫描，缺乏对称跨位置对齐；MHSA 弥补 |
...
| 主干 | DiT Transformer | DCN ResNet | DCN ResNet (复用 radarODE) | **Multi-scale + Mamba + Conformer** |
...
|--------|-------:|-----------:|
| FMCWRangeEncoder (KI) | ~17 K | 1.0% |
| PeakAuxiliaryModule (CP-1) | ~210 K | 12.4% |
| TFiLMGenerator (CP-2) | ~50 K | 2.9% |
| Multi-scale Encoder (4 conv) | ~7 K | 0.4% |
| GroupMambaBlock × 2 | ~400 K | 23.6% |
| ConformerFusionBlock | ~530 K | 31.2% |
| EMDAlignLayer (PA) | ~10 K | 0.6% |
| Regression Decoder | ~470 K | 27.7% |
| **Total (regression)** | **1,698,270** | **100%** |
