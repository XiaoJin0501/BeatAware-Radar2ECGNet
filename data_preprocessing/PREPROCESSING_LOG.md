...
**数据集**：Schellenberger et al., *Scientific Data* 7:291 (2020)
...
> ⚠️ 以上为初始默认阈值，首次运行后需查看各受试者指标分布，据此调整后重跑。
...
| 输入适配 | 方案A：Input Adapter | 方案B：1D展开 | 保持Backbone统一，仅切换前端 |
| Spec模型输入 | nperseg=64,noverlap=56 (→T≈193帧) | 更大窗口 | 足够的时间分辨率（8帧/秒）用于序列建模 |
| Spec Loss计算 | nperseg=[128,256,512] | 单分辨率 | 多尺度覆盖QRS/P-T波/RR间期，损失更全面 |
| ECG归一化 | per-segment min-max | 全局归一化 | 避免不同受试者/场景的幅度差异影响训练 |
